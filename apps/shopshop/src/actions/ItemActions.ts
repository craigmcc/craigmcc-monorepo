...
import { dbShopShop, Item } from "@repo/db-shopshop";
...
export async function createItem(data: ItemCreateSchemaType): Promise<ActionResult<Item>> {
  ...
  if (!category) {
    ...
    return { message: NO_CATEGORY_MESSAGE };
  }
  if (category.listId !== result.data.listId) {
    ...
    return { message: CATEGORY_LIST_MISMATCH_MESSAGE };
  }
...
export async function deleteItem(itemId: IdSchemaType): Promise<ActionResult<Item>> {
  ...
  if (!item) {
    ...
    return { message: NO_ITEM_MESSAGE };
  }
